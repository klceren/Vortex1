...
def get_info():
    ...
    try:
        with yt_dlp.YoutubeDL({'quiet': True, 'skip_download': True, 'force_ipv4': True, 'socket_timeout': 30, 'extractor_args': {'youtube': {'player_client': ['web', 'android']}},'http_headers': {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}}) as ydl:
...
def camera_mobile():
    # Render'da gerçek URL'i kullan, yerel modda IP:port
    render_url = os.environ.get('RENDER_EXTERNAL_URL', '').rstrip('/')
    if render_url:
        host_js = f"'{render_url}'"
    else:
        ip = get_local_ip()
        host_js = f"'https://{ip}:7861'"
...
async function startCam(){{
  ...
  st.textContent='Kamera izni bekleniyor...';
  st.className='status';
  try{{
    ...
    // play() çağrısı bazı tarayıcılarda gerekli
    ...
    // Kullanıcıya tarayıcı ayarlarını açmasını hatırlat
    if(e.name==='NotAllowedError'){{
      st.textContent='⚠ Kamera izni reddedildi. Tarayıcı ayarlarından izin ver.';
    }} else if(e.name==='NotFoundError'){{
      st.textContent='⚠ Kamera bulunamadı. Cihazda kamera var mı?';
    }}
  }}
}}
...
async function startStream(){{
  ...
  if(!stream) return;
  // Video boyutunun hazır olmasını bekle
  const vid=document.getElementById('vid');
  if(vid.videoWidth===0){{
    await new Promise(resolve=>{{
      vid.onloadedmetadata=()=>resolve();
      setTimeout(resolve,2000); // en fazla 2sn bekle
    }});
  }}
  streaming=true;
  ...
  // Canvas boyutunu bir kez sabitle
  canvas.width=vid.videoWidth||640;
  canvas.height=vid.videoHeight||480;
  const ctx=canvas.getContext('2d');
  intervalId=setInterval(async()=>{{
    if(!streaming||!stream) return;
    // Boyut değiştiyse güncelle
    if(vid.videoWidth>0 && (canvas.width!==vid.videoWidth||canvas.height!==vid.videoHeight)){{
      canvas.width=vid.videoWidth;
      canvas.height=vid.videoHeight;
    }}
    ctx.save();
    // Ön kamera için ayna efektini geri al (sunucuya normal görüntü gönder)
    ...
    canvas.toBlob(async blob=>{{
      if(!blob||blob.size<1000) return; // boş frame gönderme
...
_cam_last_time = 0  # son frame zamanı

@app.route('/camera/frame', methods=['POST'])
def camera_frame():
    global _cam_frame, _cam_last_time
    data = request.get_data()
    if not data:
        return '', 204
    with _cam_lock:
        _cam_frame = data
        _cam_last_time = _time.time()
    with _rec_lock:
    ...
    def gen():
        ...
        while True:
            with _cam_lock:
                frame = _cam_frame
                last  = _cam_last_time
            # 10 saniyeden eski frame'i "boş" say
            if frame and (_time.time() - last) < 10:
...
def _scrcpy_path():
    """scrcpy binary'sini bul — sadece Linux/local için geçerli."""
    p = shutil.which('scrcpy')
    if p:
        return p
    # Lokal Linux/Mac konumları
    for candidate in [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'scrcpy', 'scrcpy'),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'scrcpy'),
    ]:
...
def _adb_path():
    """adb binary'sini bul — sadece Linux/local için geçerli."""
    p = shutil.which('adb')
    if p:
        return p
    # Lokal Linux/Mac konumları
    for candidate in [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'scrcpy', 'adb'),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'adb'),
    ]:
        if os.path.exists(candidate):
            return candidate
    return None
...
def _is_cloud():
    """Render veya başka cloud ortamında mı?"""
    return bool(os.environ.get('RENDER') or os.environ.get('RENDER_EXTERNAL_URL'))

@app.route('/mirror/status')
def mirror_status():
    # Ekran yansıtma fiziksel USB/Wi-Fi bağlantısı gerektirir — cloud'da çalışmaz
    if _is_cloud():
        return jsonify({
            'scrcpy_found': False,
            'adb_found':    False,
            ...
            'devices':      [],
            'cloud_mode':   True,
            'message':      'Ekran yansıtma özelliği yalnızca kendi bilgisayarınızda (yerel ağda) çalışır. Render gibi cloud sunucularda USB/ADB bağlantısı mümkün değildir.'
        })
    scrcpy = _scrcpy_path()
    adb    = _adb_path()
    with _scrcpy_lock:
...
def mirror_start():
    global _scrcpy_proc
    if _is_cloud():
        return jsonify({'ok': False, 'error': 'Ekran yansıtma cloud sunucuda çalışmaz. Bu özellik sadece yerel (kendi bilgisayarınızda çalışan) sunucuda kullanılabilir.'}), 400
    ...
    with _scrcpy_lock:
        ...
        try:
            _scrcpy_proc = subprocess.Popen(cmd,
                stdout=open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'scrcpy_log.txt'), 'w'),
